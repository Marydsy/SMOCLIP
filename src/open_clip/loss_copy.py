...
class ClipLoss_copy(nn.Module):
    ...
    def forward(self, image_features, text_features, logit_scale, output_dict=False):
        ...
        # Step 3: Weighted Loss with Shapley Values 加权损失
        total_loss = 0.5 * (
        ...
        return {"contrastive_loss": total_loss} if output_dict else total_loss
class CoCaLoss_copy(ClipLoss_copy):
    def __init__(
            self,
            caption_loss_weight,
            clip_loss_weight,
...
class DistillClipLoss_copy(ClipLoss_copy):
...
class SigLipLoss_copy(nn.Module):
